...
type Trie interface {
	...
	Prove(key []byte, fromLevel uint, proofDb upsdb.Putter) error
}
...
func NewDatabase(db upsdb.Database) Database {
...
func NewDatabaseWithCache(db upsdb.Database, cache int) Database {
...
func (m cachedTrie) Prove(key []byte, fromLevel uint, proofDb upsdb.Putter) error {
