...
// Package utils contains internal helper functions for ups commands.
package utils
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses!(can search via `gups account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gups.", map[string]string{
				"host": hosttag,
			})
		}
	}
...
// new format.
//
// e.g. gups account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// gups --keystore /tmp/mykeystore --lightkdf account new
//
