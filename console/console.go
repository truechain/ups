...
import (
	...
	"github.com/truechain/ups/internal/jsre"
	"github.com/truechain/ups/internal/web3ext"
...
func (c *Console) init(preload []string) error {
	...
	flatten := "var ups = web3.ups; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = ups.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines ups.getBalance(ups.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Getrue instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Getrue metadata
	fmt.Fprintf(c.printer, "Welcome to the Getrue JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + ups.coinbase);
		console.log("at block: " + ups.blockNumber + " (" + new Date(1000 * ups.getBlock(ups.blockNumber).timestamp) + ")");
