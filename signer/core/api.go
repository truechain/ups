...
import (
	...
	"github.com/truechain/ups/accounts/usbwallet"
...
type ExternalAPI interface {
	...
	SignTransaction(ctx context.Context, args SendTxArgs, methodSelector *string) (*upsapi.SignTransactionResult, error)
...
type SignerUI interface {
	...
	OnApprovedTx(tx upsapi.SignTransactionResult)
...
func (api *SignerAPI) SignTransaction(ctx context.Context, args SendTxArgs, methodSelector *string) (*upsapi.SignTransactionResult, error) {
	var (
		err    error
	...
	response := upsapi.SignTransactionResult{Raw: rlpdata, Tx: signedTx}
...
//
// https://github.com/truechain/ups/wiki/Management-APIs#personal_sign
...
// Note, this function is compatible with ups_sign and personal_sign. As such it recovers
// the address of:
...
//
// https://github.com/truechain/ups/wiki/Management-APIs#personal_ecRecover
func (api *SignerAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid Upschain signature (V is not 27 or 28)")
	}
