...
func getBalance(client *rpc.Client, index int) {
	...
	var result string
	for i := 0; i < index; i++ {
		err := client.Call(&result, "ups_getBalance", account[i], "latest")
		if err != nil {
...
func sendTrueRawTransaction(client *rpc.Client, from string, to string, payment string, value string, fee string) (string, error) {
	...
	if err != nil {
		log.Error("ups_sendTransaction", "err", err)
	}
	return result, err
}
...
func getBalanceValue(hex string) *big.Int {
	...
	//fmt.Println("ups_getBalance Ok:", balance.Div(balance, big.NewInt(ether)), " value ", value, " hex ", hex)
	return value
}
