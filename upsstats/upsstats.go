...
// Package upsstats implements the network stats reporting service.
package upsstats
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["ups"]; info != nil {
		network = fmt.Sprintf("%d", info.(*ups.NodeInfo).Network)
		protocol = fmt.Sprintf("ups/%d", ups.ProtocolVersions[0])
	} else {
...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the upsstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to upsstats", "latency", latency)
...
func (s *Service) reportBlock(conn *websocket.Conn, block *types.Block) error {
	...
	log.Trace("Sending new block to upsstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	if s.ups != nil {
		...
		if block == nil {
			block = s.ups.BlockChain().CurrentBlock()
		}
		header = block.Header()
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(list) > 0 {
		...
		var head int64
		head = s.ups.BlockChain().CurrentHeader().Number.Int64()
	...
	for i, number := range indexes {
		...
		block := s.ups.BlockChain().GetBlockByNumber(number)
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to upsstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *websocket.Conn) error {
	...
	pending, _ := s.ups.TxPool().Stats()
	// Assemble the transaction stats and send it to the server
	log.Trace("Sending pending transactions to upsstats", "count", pending)
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	sync := s.ups.Downloader().Progress()
	syncing = s.ups.BlockChain().CurrentHeader().Number.Uint64() >= sync.HighestBlock

	price, _ := s.ups.APIBackend.SuggestPrice(context.Background())
	gasprice = int(price.Uint64())

	isCommitteeMember = s.ups.PbftAgent().IsCommitteeMember()
	isLeader = s.ups.PbftAgent().IsLeader()
	// Assemble the node stats and send it to the server
	log.Trace("Sending node details to upsstats")
	nodeStats := &nodeStats{
		Active:            true,
